...
def build_selector():
    ...
    class unet(nn.Module):
        ...
        def __init__(self, feature_scale=4, n_classes=1, is_deconv=True, in_channels=3, is_batchnorm=True):
            ...
            self.final = nn.Conv2d(filters[6], n_classes, 1)
...
class dc_invase():
    def __init__(self):
        ...
        self.patch_shape = (64,64)
        self.batch_size = 1
        self.img_mean = [0,0,0]#[0.485, 0.456, 0.406]
        self.img_std = [1,1,1]#[0.229, 0.224, 0.225]
        self.alpha = 1
        self.beta = 0.05
        self.exp_name = 'Weights/dci_patchwise_64x64_oxford_exp'
        ...
        self.optimizer_pred = optim.Adam(self.predictor.parameters(), lr=1e-4, betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-5, amsgrad=False)
        self.optimizer_dis = optim.Adam(self.discriminator.parameters(), lr=1e-5, betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-5, amsgrad=False)
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for phase in ['train', 'valid']:
                ...
                print('{} Base_Loss: {:.4f} Sel_Loss: {:.4f} Pred_Loss: {:.4f} Dis_Loss: {:.4f} Spa: {:.4f} BAC: {:.4f} PAC: {:.4f} DAC: {:.4f} IoU: {:.4f}'.format(
                    phase, epoch_base_loss, epoch_sel_loss, epoch_pred_loss, epoch_dis_loss, epoch_spa, epoch_base_acc, epoch_pred_acc, epoch_dis_acc,epoch_iou))
