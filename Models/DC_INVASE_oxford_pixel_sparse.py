            ...
            for phase in ['train', 'valid']:
                ...
                for sampled_batch in self.dataloaders[phase]:
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        dis_loss = F.cross_entropy(dis_out,1-labels)
...
#                         int_1_l1_loss = self.l1_loss(int_1,torch.zeros(int_1.shape).to(self.device))
                        ...
                        int_4_l1_loss = torch.mean(int_4)
                        int_3_l1_loss = torch.mean(int_3)
                        int_2_l1_loss = torch.mean(int_2)
                        int_1_l1_loss = torch.mean(int_1)
                ...
                print('{} Base_Loss: {:.4f} Sel_Loss: {:.4f} Pred_Loss: {:.4f} Dis_Loss: {:.4f} Spa: {:.4f} Int_Spa: {:.4f} BAC: {:.4f} PAC: {:.4f} DAC: {:.4f} IoU: {:.4f}'.format(
                    phase, epoch_base_loss, epoch_sel_loss, epoch_pred_loss, epoch_dis_loss, epoch_spa, epoch_int_spa, epoch_base_acc, epoch_pred_acc, epoch_dis_acc, epoch_iou))
                # deep copy the model
